    ...
    if(categoryType == "All")
      {
        return StreamBuilder<QuerySnapshot>(
          stream: Firestore.instance.collection("users")
              ...
              .collection("poultryData").orderBy("timestamp", descending: true).snapshots(),
    ...
    else
      {
        return StreamBuilder<QuerySnapshot>(
          stream: Firestore.instance.collection("users")
              .document(MyPoultry.sharedPreferences.getString(MyPoultry.userUID))
              .collection("poultryData").where("name", isEqualTo: categoryType).orderBy("timestamp", descending: true).snapshots(),
          builder: (BuildContext context, snapshot) {
            if(!snapshot.hasData)
            {
              return circularProgress();
